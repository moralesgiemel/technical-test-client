 <template>
  <v-dialog v-model="editValueDialog" persistent max-width="600px">
...
export default {
  ...
  methods: {
    editValue() {
      ...
      console.log(payload);
      this.$http
        .put(`${process.env.VUE_APP_API}/values/edit`, { ...payload })
        .then((response) => {
          if (response.data) {
            this.success = true;
          }
          ...
          response.data
            ? this.$swal.fire(this.$results.result.valueEditSuccess)
